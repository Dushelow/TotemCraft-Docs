...
Стоп-факторы (✋ только вручную): бан или мут когда-либо (ник, прошлые ники), твинк (у TG ID уже был
одобренный аккаунт; IP не смотрим нигде — через прокси в России он общий), блок в боте, брань и символика в нике, пароле
...
def decide(f):
    ...
    if f.get('approved_before'):
        banned = set(f.get('approved_before_banned') or [])
        stop.append("Твинк: с этого Telegram уже одобрен аккаунт "
                    + ", ".join(f"{n} (в бане)" if n in banned else n for n in f['approved_before'][:3]))
    if f.get('blocked'):
