...
- При одобрении - автоматически выполнить `authme register <ник> <пароль>` на сервере через RCON (в канал консоли Discord уходит копия с паролем звёздочками)
...
bot.py
  ...
  ├── отправляет authme-команду в консоль сервера (RCON), копию без пароля в Discord
...
Второй вебхук в канале консоли сервера (DiscordSRV). Бот пишет туда строку `authme register <ник> ********`, чтобы было видно, кого зарегистрировали. Саму команду DiscordSRV от вебхука не выполняет: регистрация идёт через RCON (`RCON_PASSWORD` в `.env`, `enable-rcon=true` и тот же `rcon.password` в `server.properties`).
...
RCON_PASSWORD=сюда_rcon.password_из_server.properties
```
