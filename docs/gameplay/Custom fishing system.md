...
Единственный путь к копии предмета теперь: трофей выдан, а `set_lore_applied` до бота не дошёл. Поэтому отправка повторяется 3 раза, а после неудачи в лог пишется `SEVERE` с fish_id — флаг выставить руками.
