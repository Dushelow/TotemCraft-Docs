...
- `discord_id → NULL`, `claimed → 0` при `transfer_fish()` (новый владелец ещё не клеймил). `lore_applied` передача не трогает (с 1.2.2): предмет остаётся у отправителя
...
5. ПЕРЕДАЧА (опционально)
   ...
   При принятии:
   ├── transfer_fish() → current_owner=to, claimed=0, discord_id=NULL (lore_applied не меняется)
...
Флаг `lore_applied = 1` означает, что предмет с lore **существует в игре** — у кого угодно, не обязательно у текущего владельца. Флаг сбрасывается только при `set_fish_released(True)`: предмет стал ведром.

Правило: **предмет с одним fish_id существует в игре в одном экземпляре.** Все проверки ниже держат именно его.

`FishCatchListener.scheduleLookupsAsync` накладывает lore на свежепойманную рыбу, только если `GET /fwhere` ответил и одновременно: `lore_applied=0`, `status != released`, `current_owner` = поймавший. Иначе пропуск с записью причины в лог.

- `lore_applied=1` — игрок выбросил трофей и поймал рыбу того же вида: без проверки старый трофей лёг бы на новую рыбу.
- `released` — до 1.2.2 проверки не было. Выпуск в ведро сбрасывает `lore_applied`, а `/fish_lookup_latest?recent=1` отдаёт трофей 90 секунд после поимки, не глядя на выпуск. Игрок убирал ведро в сундук, ловил рыбу того же вида — и получал второй трофей с тем же ID. Воспроизведено на стенде.
- чужой владелец — трофей передали в эти 90 секунд.
- бот не ответил — lore не накладываем: трофей потом можно получить через `/claim`, копию уже не отменить.

### Передача и предмет

Передача (`transfer_fish`) меняет только владельца в БД. Предмет остаётся у отправителя, пока тот не отдаст его руками.

До 1.2.2 `transfer_fish` сбрасывал `lore_applied=0`, и `/claim` получателя делал трофей из любой обычной рыбы при живом оригинале у отправителя: две копии. Воспроизведено на стенде.

Теперь `/claim` при `lore_applied=1`, если трофея с этим ID нет в инвентаре, клеймит рыбу в боте (очки, привязка Discord), но предмет не создаёт и пишет «попроси передать». Рыба в инвентаре для этого не нужна. Если отправитель уже отдал предмет — обычный клейм.

Передачи, сделанные ботом до 1.2.2, остались с `lore_applied=0`: их при выкладке выставить в `1` вручную (кроме выпущенных).

### Рыба не попала в инвентарь (воронка, AFK-ферма, полный инвентарь)

С 1.2.1 `POST /set_lore_applied` отправляется **только если предмет с lore реально создан** (`applyLoreToInventory` и `giveTrophy` возвращают `true`).

До 1.2.1 флаг поднимался всегда. Воронка или вагонетка с воронкой забирала пойманную рыбу раньше игрока, lore не на что было наложить, а `lore_applied` всё равно становился `1`. Бот к этому моменту уже авто-клеймил трофей, и `/claim` отвечал «Эта рыба уже заклеймлена»: трофей пропадал навсегда. Та же ловушка была в `/claim` без места в инвентаре — он обещал «получить позже через /claim», но позже упирался в тот же флаг.

Теперь `/claim` выдаёт уже заклеймленный трофей, если `GET /fwhere` отвечает: `status=claimed`, `lore_applied=0`, `current_owner` = игрок. Повторного клейма в боте нет. Выпущенную рыбу (`status=released`) так не выдать: она живёт в мире или в ведре.

Единственный оставшийся путь к копии предмета: трофей выдан, а `set_lore_applied` до бота не дошёл. Поэтому отправка повторяется 3 раза, а после неудачи в лог пишется `SEVERE` с fish_id — флаг выставить руками.
...
`transfer_fish()` устанавливает `claimed=0, discord_id=NULL`. Новый владелец должен сделать `/claim <id>` чтобы привязать рыбу к своему Discord и получить очки в лидерборде. Что при этом происходит с предметом — раздел «Передача и предмет» выше.
