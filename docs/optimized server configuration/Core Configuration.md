...
| world-settings.default.ticks-per.hopper-transfer | 8 | 8 | Ваниль. Было 3: это ускоряло воронки почти втрое, а не замедляло, и сортировщики на компараторе забирали по 2 предмета (жалоба LAVeBlaze, стенд 18.09.2026). Не уменьшать. |
| world-settings.default.ticks-per.hopper-check | 3 | 1 | Воронки реже проверяют, что забрать, → +TPS. На сортировщики не влияет (проверено на стенде). |
